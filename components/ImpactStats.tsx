    ...
    <section className="py-24 bg-orange-50 text-gray-900 print-break-inside-avoid">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {stats.map((s, i) => {
            const isRevenue = s.label === 'ECサイト月間売上';
            return (
              <div key={i} className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border border-orange-100 flex flex-col h-full transform transition-all hover:-translate-y-1">
                ...
                {/* Impact Box: Tightly centered for 3 items, spread only for revenue */}
                <div className={`flex items-center ${isRevenue ? 'justify-between' : 'justify-center gap-x-4 sm:gap-x-8 md:gap-x-16'} bg-orange-50/50 rounded-2xl p-4 md:p-6 mb-6 border border-orange-100/50 overflow-hidden`}>
                  
                  {/* Before Block */}
                  <div className={`${isRevenue ? 'text-left' : 'text-center'} flex-shrink-0 transform scale-90 sm:scale-100 origin-center`}>
                  ...
                  {/* After Block */}
                  <div className={`${isRevenue ? 'text-right' : 'text-center'} flex-shrink-0 min-w-0`}>
                    <div className="text-[10px] text-[#f97316] font-bold mb-1 uppercase tracking-tighter">After</div>
                    <div className={`${isRevenue ? 'text-[7.5vw] md:text-5xl' : 'text-[8.5vw] md:text-5xl'} sm:text-4xl font-black text-[#f97316] tracking-tighter leading-none`}>
                ...
                {/* Description Paragraph: Normal wrap on mobile, 0.9rem font on PC */}
                <p className="text-xs sm:text-sm md:text-[0.9rem] text-gray-600 font-bold leading-relaxed border-l-4 border-orange-200 pl-4 py-1">
                  {s.desc}
                </p>
              </div>
            );
